package com.plenome.pos.views.horizontal;
...
public class HorizontalPicker extends LinearLayout implements HorizontalPickerListener {
    ...
    public void init() {
        ...
        tvMonth.setTextColor(mMonthAndYearTextColor != -1 ? mMonthAndYearTextColor : getColor(R.color.primaryTextColor_horizontal));
        tvToday.setVisibility(showTodayButton ? VISIBLE : INVISIBLE);
        tvToday.setTextColor(mTodayButtonTextColor != -1 ? mTodayButtonTextColor : getColor(R.color.colorPrimary_horizontal));
        ...
        mDateSelectedColor = mDateSelectedColor == -1 ? getColor(R.color.colorPrimary_horizontal) : mDateSelectedColor;
        mDateSelectedTextColor = mDateSelectedTextColor == -1 ? Color.WHITE : mDateSelectedTextColor;
        mTodayDateTextColor = mTodayDateTextColor == -1 ? getColor(R.color.primaryTextColor_horizontal) : mTodayDateTextColor;
        mDayOfWeekTextColor = mDayOfWeekTextColor == -1 ? getColor(R.color.secundaryTextColor_horizontal) : mDayOfWeekTextColor;
        mUnselectedDayTextColor = mUnselectedDayTextColor == -1 ? getColor(R.color.primaryTextColor_horizontal) : mUnselectedDayTextColor;
        rvDays.init(
                getContext(),
                finalDays,
                finalOffset,
        ...
        tvMonth.setText(currentDate.toString(mMonthPattern.isEmpty() ? "MMMM yyyy" : mMonthPattern));
    }
    ...
    private void changeMonth(int increment) {
        ...
        updateMonthDisplay();
        rvDays.setDate(currentDate);

        DateTime now = DateTime.now();
        if (currentDate.getYear() == now.getYear() && currentDate.getMonthOfYear() == now.getMonthOfYear()) {
            btnPreviousMonth.setVisibility(View.GONE);
        } else {
            btnPreviousMonth.setVisibility(View.VISIBLE);
        }
    }

    private void updateMonthDisplay() {
        DateTimeFormatter formatter = DateTimeFormat.forPattern(mMonthPattern.isEmpty() ? "MMMM yyyy" : mMonthPattern);
        tvMonth.setText(currentDate.toString(formatter));
    }

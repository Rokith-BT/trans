import { Box, Button, AutoComplete, CustomDatePicker, CustomDialog, Select, Text, Input } from '@/atoms';
...
export type FilterOption = {
  ...
  fieldType?: 'select' | 'date' | 'text' | 'auto_select';
};
...
interface RecipientFilterProps {
  ...
  isRecipientFilter?: boolean;
}

export const GenericFilter: React.FC<RecipientFilterProps> = ({
  open,
  onClose,
  filters,
  onApply,
  ...
  return (
    <CustomDialog open={open} onClose={onClose} maxWidth={filters.length > 1 ? 'sm' : 'xs'}>
      <Box className="py-[10px]">
        ...
        <Box className="flex flex-wrap ">
          {filters?.map((filter) => {
            ...
            return (
              <Box key={filter.key} className={`flex p-3 ${filters.length > 1 ? 'w-1/2' : 'w-full'}`}>
                {type === 'select' && (
                  <Select
                ...
                {type === 'text' && (
                  <Input
                    label={filter.label}
                    name={filter.key}
                    value={dialogFilters[filter.key] || null}
                    onChange={(e) => handleFilterChange(filter.key, e?.target.value as string | null)}
                    fullWidth
                  />
                )}

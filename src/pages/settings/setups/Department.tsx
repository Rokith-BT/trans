...
const Department: React.FC<OrganProps> = () => {
  ...
  const {
    state: { departments, count ,loading},
  ...
  }, [openDialog, openDepDialog, openRestore, openDeleteDialog]);
  useEffect(() => {
    const newLocation = {
      ...location
    };
  ...
  const columns = [
    ...
    { headerName: 'Department', field: 'name', cellStyle: rowStyle },
    {
      headerName: 'Status',
  ...
  return (
    <Box className="p-4 sm:p-6 md:p-8">
      <Box className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <Text className="!text-[19px] sm:!text-[20px] !font-bold text-[#804595]">Department</Text>
      ...
      <Box className="mt-4 overflow-x-auto">
        ...
        <SetupDataTable rowColoum={columns} rowData={departments} />
      </Box>
      <SetupDialog open={openDialog} onClose={() => SetOpenDialog(false)} fieldName="Department" />
      <SetupEditDialog
        open={openDepDialog}
        onClose={() => SetOpenDepDialog(false)}
        fieldName="Department"
        id={id}
        nameValue={name}
      />
      <DeleteDialog open={openDeleteDialog} onClose={() => SetOpenDeleteDialog(false)} id={id} fieldName="Department" />
      <RestoreDialog open={openRestore} onClose={() => setOpenRestore(false)} id={id} fieldName="Department" />
    </Box>
  );
};
